...
10.Add a Member
11) Add a Car
12) Assign Car to Member (Assume valid data is supplied)
13) Cars Owned: Based on the member ID print Number of cars & Registration Numbers
*/
public class Main {
	...
	public static void carsOwned(String memberId, ArrayList<Member> memberList) {
		...
		Member reqmem = null;
		ArrayList<MemberCar> reqcarList = new ArrayList<MemberCar>();
		...
		if (reqmem != null) {
			reqcarList = reqmem.getCarList();
			if (!reqcarList.isEmpty()) {
				for (MemberCar mcar : reqcarList) {
					str.add(mcar.getRegNum());
				}
				System.out.println("Number of cars : " + reqcarList.size()
			...
			System.out.println("NO Memeber Exists With id : " + id);
		}
	}
	...
	public static Car addCar(BufferedReader br) {
		...
		try {
			...
			c = new Car(Long.parseLong(id), name, model, Integer.parseInt(makeYear), company,
					Integer.parseInt(comfortLevel));
	...
	public static void initData(ArrayList<Member> memberList, ArrayList<Car> carList) {
		SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
		try {
			memberList.add(new Member(1L, "joe", "root", "joe.root@a.com", "1234567890", "AH1", sdf.parse("12-12-2001"),
					sdf.parse("12-12-2010")));
			memberList.add(new Member(2L, "ben", "stokes", "ben.stokes@a.com", "2345678901", "AH2",
					sdf.parse("12-12-2002"), sdf.parse("12-12-2011")));
			memberList.add(new Member(3L, "virat", "kohli", "virat.kohli@a.com", "3456789012", "AH3",
					sdf.parse("12-12-2003"), sdf.parse("12-12-2012")));
